...
- `business-context-v2` 合成知识层包含 10 条电商术语、4 张表全部 17 个字段备注，以及与冻结评测
  8 条成功案例逐条一致的可禁用训练对。SQL generator 只注入命中术语的相关备注，并以规范化字符
  二元组 Jaccard `0.72` 为门、最多召回 2 条相似只读模板；它不含真实企业知识、向量库、枚举值
  索引、记忆或技能，模板也不替代 schema 校验或机械安全边界。
