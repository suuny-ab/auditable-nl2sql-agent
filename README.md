...
回执绑定为可重算 SHA-256 指纹的 `evidence-v1`。高行数和写操作查询会在执行前持久化挂起，
可使用同一 run ID 在另一进程批准或拒绝；批准仍不能绕过机械只读边界。

当前 SQL stub 只是可重复的工作流替身，不构成真实 NL2SQL 能力或正确率主张。真实模型、
自然语言回答、API、网页、评测和 Docker 仍待实现。SHA-256 用于稳定绑定和变化检测，不是
数字签名，也不证明 SQL 或答案的业务语义正确。
