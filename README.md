...
SQL generator 还会从版本化合成知识资源中按问题匹配业务术语，只把命中术语的定义和相关字段
备注加入本次生成上下文；同时用规范化字符二元组 Jaccard 召回不超过 2 条相似训练对，阈值固定
为 `0.72`。8 条训练对逐条来自冻结评测成功案例并带 `enabled` 开关；它们只是只读 SQL 参考模板，
不会替代 schema 校验、审批门或数据库机械只读边界。知识层不含真实企业知识、向量库、枚举索引、
用户记忆或技能。
