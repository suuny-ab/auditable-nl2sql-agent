...
SQL generator 还会从两份版本化合成知识文件中按问题匹配业务术语，只把命中术语的定义和相关
字段备注加入本次生成上下文；未命中的知识不会整库塞进 prompt。当前知识层包含 10 条电商术语
和 `customers/products/orders/order_items` 全部 17 个字段备注，不含真实企业知识、枚举索引、
用户记忆或技能。
