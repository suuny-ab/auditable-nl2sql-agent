...
> 事实快照：`main@d50f510214fbc1f57cf42da467df28f473a76cd1`（2026-08-03）
>
...
| `8/15` | 第二套 schema 最新唯一轮次的答案正确数；五类均按原合同判定 | 表/字段零重合、状态与渠道改码、金额用整数分的第二合成库 15 题 | 摘要候选只完整运行一次；结果与上一轮同为 `8/15` | 不是 schema 泛化成功；摘要没有提分 | [报告](work/schema-summary-injection.md) · [PR #30](https://github.com/suuny-ab/auditable-nl2sql-agent/pull/30) |
| `0/7` | 上述 15 题中，七条预期 `success` 案例的正确数 | 第二合成库的 7 条成功题 | 最新轮次已封存，不补跑 | 不是“七题都安全失败”；其中两条执行了只读 SQL，但结果/审批合同不符 | [报告](work/schema-summary-injection.md) · [PR #30](https://github.com/suuny-ab/auditable-nl2sql-agent/pull/30) |
...
三个分母和观测条件不同，不能平均、相减或拼成一个“综合泛化分”。尤其是 `40/40` 与 `0/7`
同时成立：前者是已见主库开发集，后者是换 schema 成功题短板。
...
| 最多 `15` 次、重试 `0` | 第二库 15 题各至多一次 | 换 schema HOLDOUT / 复测 | 每个授权切片一个完整轮次 | 不能把未用调用结转或用来刷分 | [报告](work/schema-summary-injection.md) · [PR #30](https://github.com/suuny-ab/auditable-nl2sql-agent/pull/30) |
...
| 最多 `8` 个 | 问题命中的规范枚举值按稳定顺序截断 | 合成枚举过滤提示 | 确定性合同，可离线复算 | 不是数据库运行时扫描，也不是每题必有 8 个命中 | [报告](work/enum-value-index.md) · [PR #22](https://github.com/suuny-ab/auditable-nl2sql-agent/pull/22) |
...
| Python `120/120` | `unittest discover` 在锁定依赖环境运行的全部产品与合同测试 | `api/tests` | 每个提交可重复运行 | 测试通过不等于模型正确率或生产可靠性 | [报告](work/showcase-validation-arc.md) · [PR #31](https://github.com/suuny-ab/auditable-nl2sql-agent/pull/31) |
...
| CI `3` 路 | GitHub Actions 分别运行 api / web / container job | PR 与 main | 每次 push 重新运行 | 三路绿不证明 v2 已部署 | [工作流](../.github/workflows/ci.yml) · [PR #31](https://github.com/suuny-ab/auditable-nl2sql-agent/pull/31) |

## 复述时的最短安全版本

> 主库已见开发集从 `14/20` 调到 `40/40`；换 schema 最新只有 `8/15`，成功题仍 `0/7`；完整
> 改述基线的正确率与稳定率都是 `24/30`，三题修复后只有正确率投影 `27/30`，没有新的完整稳定率。
> 所有真实评测均单轮、自动重试 `0`，测试规模是 Python `120/120`、Web `3/3`；这些都只针对
